...
echo 'PROCESO DE TIMBRADO PARA "NOTA DE CRÉDITO". CFDI VERSIÓN 3.3';
echo '</div>';    
...
### 4. DATOS GENERALES DE LA FACTURA ##############################################
    ...
    $fact_tipcompr     = "E";                             // 4.4 Tipo de comprobante.
    ...
    $UUID_DelDocRel = "A39DA66B-52CA-49E3-879B-5C05185B0EF7";  // 4.20 UUID del documento relacionado.
...
### 6. ARRAYS QUE CONTIENEN LOS ARTICULOS QUE FORMAN PARTE DE LA VENTA #####################
    ...
    $Array_ClaveProdServ    = ['23241610', '20142900', '31151600']; // 6.1 Clave del SAT correspondiente al artículo o servicio (consultar el catálogo de productos del SAT).
    $Array_NoIdentificacion = ['CORCOB', 'TANQG-BUT', 'CADGALV'];   // 6.2 Clave asignada al artículo o servicio, sistema local.
    $Array_Cantidad         = ['5', '3', '3.5'];                    // 6.3 Cantidad.
    $Array_ClaveUnidad      = ['H87', 'H87', 'LM'];                 // 6.4 Clave del SAT correspondiente a la unidad de medida (consultar el catálogo de productos del SAT).
    $Array_Unidad           = ['PIEZA', 'PIEZA', 'METRO'];          // 6.5 Descripción de la unidad de medida.
    $Array_Descripcion      = ['CORTADOR DE TUBO DE COBRE', 'TANQUE CON GAS BUTANO', 'CADENA GALVANIZADA']; // 6.6 Descripción del artículo o servicio.
    $Array_ValorUnitario    = ['125.50', '35.80', '131.00'];        // 6.7 Valor unitario del artículo o servicio.
    $Array_Importe          = ['627.50', '107.40', '458.50'];       // 6.8 Importe del artículo o servicio.
    $Array_Descuento        = ['0', '0', '0'];                      // 6.9 Descuento aplicado al artículo o servicio.
    
    
 ### 7. ARRAYS QUE CONTIENEN LOS IMPUESTOS TRASLADADOS Y RETENIDOS POR CONCEPTO #############

    // Trasladados.
    $ArrayTraslado_Base = ['627.50', '107.40', '458.50', '80.00'];                 // 7.1 Atributo requerido para señalar la base para el cálculo del impuesto, la determinación de la base se realiza de acuerdo con las disposiciones fiscales vigentes. No se permiten valores negativos
    ...
    $ArrayTraslado_TipoFactor = ['Tasa', 'Tasa', 'Tasa', 'Tasa'];                  // 7.3 Atributo requerido para señalar la clave del tipo de factor que se aplica a la base del impuesto (consultar catálogos del SAT).
...
### 11. CREACIÓN Y ALMACENAMIENTO DEL ARCHIVO .XML (CFDI) ANTES DE SER TIMBRADO ###################
    ...
    cargaAtt($DocsRel, array("TipoRelacion"=>"01"));
...
$Impuestos = $root->appendChild($Impuestos);
    ...
    $cadena_original .= "|";   
        ...
        $file = fopen($SendaCFDI."CadenaOriginal_NotCred_".$NoFac.".txt", "w");
        fwrite($file, $cadena_original . PHP_EOL);
        fclose($file);
        chmod($SendaCFDI."CadenaOriginal_NotCred_".$NoFac.".txt", 0777);      
    ...
    $NomArchCFDI = $SendaCFDI."PreCFDI-33_NotCred_".$NoFac.".xml";
...
## 13. PROCESOS POSTERIORES AL TIMBRADO ########################################
    ...
    $VarXML->save($SendaCFDI."RespServ_NotCred_".$NoFac.".xml");
    chmod($SendaCFDI."RespServ_NotCred_".$NoFac.".xml", 0777);
    ...
    foreach($RespServ as $Nodo){
        ...
        if($valor_del_nodo != ""){
            ...
            $NomArchXML = "CFDI-33_NotCred_".$NoFac.".xml";
            $NomArchPDF = "CFDI-33_NotCred_".$NoFac.".pdf";
...
//                file_put_contents($file_name, "\xEF\xBB\xBF".$f);
            ...
            $docXML->load($SendaCFDI."CFDI-33_NotCred_".$NoFac.".xml");
            ...
            <html>
                ...
                <body>
                    ...
                    <input type="button" value="Imprimir nota de crédito" onclick="ImprimirFact()" />
                    &nbsp;&nbsp;
                    ...
                    <script type="text/javascript">
                        ...
                        function ImprimirFact(){
                            window.open("pdf_NotaDeCredito.php?NomArchXML=<?php echo $NomArchXML ?>&NomArchPDF=<?php echo $NomArchPDF ?>","_blank");
                        }
