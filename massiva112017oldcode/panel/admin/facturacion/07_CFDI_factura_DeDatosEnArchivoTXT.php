...
# Obteniendo datos del archivo .TXT ############################################    
    
$SendaTXT   = "archs_txt/";   // Directorio en donde se almacenan los archivos .txt con datos a timbrar.
$NomArchTXT = "DatosFactura.txt"; // Nombre del archvo .TXT con datos a timbrar.
$DatosArchTXT = file_get_contents($SendaTXT.$NomArchTXT); // Carga del contenido del archivo .TXT
$DatosArchTXT = preg_replace("[\n|\r|\n\r]", "", $DatosArchTXT); // Quitando retornos de carro, saltos de línea, etc.

$ObjJSON = json_decode($DatosArchTXT); 
...
### 4. DATOS GENERALES DE LA FACTURA ##################################################
    $fact_serie        = $ObjJSON->{'NoSerie'};   // 4.1 Número de serie.
    $fact_folio        = $ObjJSON->{'NoFolio'};   // 4.2 Número de folio (para efectos de demostración se asigna de manera aleatoria).
    $NoFac             = $fact_serie.$fact_folio; // 4.3 Serie de la factura concatenado con el número de folio.
    $fact_tipcompr     = $ObjJSON->{'TipoCompr'}; // 4.4 Tipo de comprobante.
    ...
    $NumCtaPago        = $ObjJSON->{'NumCtaPago'};        // 4.11 Número de cuenta (sólo últimos 4 dígitos, opcional).
    $condicionesDePago = $ObjJSON->{'condsDePago'};       // 4.12 Condiciones de pago.
    $formaDePago       = $ObjJSON->{'formaDePago'};       // 4.13 Forma de pago.
    $metodoDePago      = $ObjJSON->{'metodoDePago'};      // 4.14 Clave del método de pago. Consultar catálogos de métodos de pago del SAT.
    $TipoCambio        = $ObjJSON->{'TipoCambio'};        // 4.15 Tipo de cambio de la moneda.
    $LugarExpedicion   = $ObjJSON->{'LugarExpedicion'};   // 4.16 Lugar de expedición (código postal).
    $moneda            = $ObjJSON->{'moneda'};            // 4.17 Moneda
...
$Ind = 0;    
foreach ($ObjJSON->{'Articulos'} as $Elemento){
    $Array_ClaveProdServ[$Ind]    = $Elemento->{"ClaveProdServ"};    // 6.1 Clave del SAT correspondiente al artículo o servicio (consultar el catálogo de productos del SAT).
    $Array_NoIdentificacion[$Ind] = $Elemento->{"NoIdentificacion"}; // 6.2 Clave asignada al artículo o servicio, sistema local. 
    $Array_Cantidad[$Ind]         = $Elemento->{"Cantidad"};         // 6.3 Cantidad.
    $Array_ClaveUnidad[$Ind]      = $Elemento->{"ClaveUnidad"};      // 6.4 Clave del SAT correspondiente a la unidad de medida (consultar el catálogo de productos del SAT).
    $Array_Unidad[$Ind]           = $Elemento->{"Unidad"};           // 6.5 Descripción de la unidad de medida.
    $Array_Descripcion[$Ind]      = $Elemento->{"Descripcion"};      // 6.6 Descripción del artículo o servicio.
    $Array_ValorUnitario[$Ind]    = $Elemento->{"ValorUnitario"};    // 6.7 Valor unitario del artículo o servicio.
    $Array_Importe[$Ind]          = $Elemento->{"Importe"};          // 6.8 Importe del artículo o servicio.
    $Array_Descuento[$Ind]        = $Elemento->{"Descuento"};        // 6.9 Descuento.
    $Ind++;
}    
...
$Ind = 0;    
foreach ($ObjJSON->{'ImportesTrasladados'} as $Elemento){
    $ArrayTraslado_Base[$Ind]       = $Elemento->{"Base"};       // 7.1 Atributo requerido para señalar la base para el cálculo del impuesto, la determinación de la base se realiza de acuerdo con las disposiciones fiscales vigentes. No se permiten valores negativos
    $ArrayTraslado_Impuesto[$Ind]   = $Elemento->{"Impuesto"};   // 7.2 Atributo requerido para señalar la clave del tipo de impuesto trasladado aplicable al concepto (consultar catálogos del SAT).
    $ArrayTraslado_TipoFactor[$Ind] = $Elemento->{"TipoFactor"}; // 7.3 Atributo requerido para señalar la clave del tipo de factor que se aplica a la base del impuesto (consultar catálogos del SAT).
    $ArrayTraslado_TasaOCuota[$Ind] = $Elemento->{"TasaOCuota"}; // 7.4 Atributo condicional para señalar el valor de la tasa o cuota del impuesto que se traslada para el presente concepto. Es requerido cuando el atributo TipoFactor tenga una clave que corresponda a Tasa o Cuota (consultar catálogos del SAT).
    $ArrayTraslado_Importe[$Ind]    = $Elemento->{"Importe"};    // 7.5 Atributo condicional para señalar el importe del impuesto trasladado que aplica al concepto. No se permiten valores negativos. Es requerido cuando TipoFactor sea Tasa o Cuota
    $Ind++;
}    
...
### 9. DATOS GENERALES DEL EMISOR #################################################  
    ...
    $emisor_rs     = $ObjJSON->{'emisor_RazSoc'};  // 11 // 9.1 Nombre o Razón social.
    $emisor_rfc    = $ObjJSON->{'emisor_RFC'};                        // 12 // 9.2 RFC (al momento de timbrar el SAT comprueba que el RFC se encuentre registrado y vigente en su base de datos)
    $emisor_regfis = $ObjJSON->{'emisor_ClaRegFis'}; // 13 // 9.3 Régimen fiscal.    
        
    
### 10. DATOS GENERALES DEL RECEPTOR (CLIENTE) #####################################
    
    $RFC_Recep = $ObjJSON->{'receptor_RFC'};                                                 // 14 // 10.1 RFC (al momento de timbrar el SAT comprueba que el RFC se encuentre registrado y vigente en su base de datos).
    ...
    $receptor_rs  = $ObjJSON->{'receptor_RazSoc'};                                           // 15 // 10.4 Nombre o razón social.
...
//    chmod($SendaCFDI."CadenaOriginal_Factura_".$NoFac.".txt", 0777);      
    ...
    # 11.7 EJEMPLO D AGREGAR UNA ADDENDA, PROCESO OPCIONAL, NO NECESARIO PARA TIMBRAR UN CFDI #####
