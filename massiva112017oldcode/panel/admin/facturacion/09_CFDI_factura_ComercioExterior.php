...
echo 'PROCESO DE TIMBRADO PARA "FACTURAR". CFDI VERSIÓN 3.3 CON COMPLEMENTO PARA COMERCIO EXTERIOR.';
echo '</div>';    
...
### 4. DATOS GENERALES DE LA FACTURA ##############################################
    $fact_serie        = "C";                             // 4.1 Número de serie.
...
// 11.5.1 COMPLEMENTO PARA COMERCIO EXTERIOR =====================================
            
    $complemento = $xml->createElement("cfdi:Complemento");
    $complemento = $root->appendChild($complemento);
            
            
    $ComExt = $xml->createElement("cce11:ComercioExterior");
    $ComExt = $complemento->appendChild($ComExt);
    cargaAtt($ComExt, array(
            ...
            "Incoterm"=>"DAT",
            "Subdivision"=>"0",
            "TipoCambioUSD"=>"18.836200",
            "TotalUSD"=>"28556.60"
        )
    );


    $emisorComExt = $xml->createElement("cce11:Emisor");
    $emisorComExt = $ComExt->appendChild($emisorComExt);

        $DomEmisorComExt = $xml->createElement("cce11:Domicilio");
        $DomEmisorComExt = $emisorComExt->appendChild($DomEmisorComExt);

        cargaAtt($DomEmisorComExt, array(
                                "Calle"=>"PERIFERICO SUR",
                                "NumeroExterior"=>"#4194 #PB2",
                                "Colonia"=>"0347",
                                "Localidad"=>"01",
                                "Municipio"=>"010",
                                "Estado"=>"DIF",
    ...
    $receptorComExt = $xml->createElement("cce11:Receptor");
    $receptorComExt = $ComExt->appendChild($receptorComExt);

        $DomReceptorComExt = $xml->createElement("cce11:Domicilio");
        $DomReceptorComExt = $receptorComExt->appendChild($DomReceptorComExt);

        cargaAtt($DomReceptorComExt, array(
                                "Calle"=>"2100 West Loop South",
                                "Estado"=>"TX",
                                "Pais"=>"USA",
                                "CodigoPostal"=>"77027"
                                )
    ...
    $DestinComExt = $xml->createElement("cce11:Destinatario");
    $DestinComExt = $ComExt->appendChild($DestinComExt);
    
    cargaAtt($DestinComExt, array(
                        "NumRegIdTrib"=>"821816654",
                        "Nombre"=>"Trade Group Inc."
                        )
                    );

        $DomDestinComExt = $xml->createElement("cce11:Domicilio");
        $DomDestinComExt = $DestinComExt->appendChild($DomDestinComExt);

            cargaAtt($DomDestinComExt, array(
                                "Calle"=>"412 NAFTA Blvd.",
                                "Estado"=>"TX",
                                "Pais"=>"USA",
                                "CodigoPostal"=>"78045"
                                )
    ...
    $Mercancias = $ComExt->appendChild($Mercancias);
    
    for ($i=0; $i<count($ArrayMerc_NoIdentificacion); $i++){
    
        $Merc = $xml->createElement("cce11:Mercancia");
        $Merc = $Mercancias->appendChild($Merc);

            cargaAtt($Merc, array(
                                "NoIdentificacion"=>$ArrayMerc_NoIdentificacion[$i],
                                "FraccionArancelaria"=>$ArrayMerc_FraccionArancelaria[$i],
                                "CantidadAduana"=>$ArrayMerc_CantidadAduana[$i],
                                "UnidadAduana"=>$ArrayMerc_UnidadAduana[$i],
                                "ValorUnitarioAduana"=>$ArrayMerc_ValorUnitarioAduana[$i],
                                "ValorDolares"=>$ArrayMerc_ValorDolares[$i]
                                )
                            );            
...
// FIN DE COMPLEMENTO DE COMERCIO EXTERIOR =====================================    
    
    
    #== 11.6 Termina de conformarse la "Cadena original" con doble ||
    $cadena_original .= "|";   
    
        $file = fopen($SendaCFDI."CadenaOriginal_Factura_ComExt_".$NoFac.".txt", "w");
        fwrite($file, $cadena_original . PHP_EOL);
        fclose($file);
        chmod($SendaCFDI."CadenaOriginal_Factura_ComExt_".$NoFac.".txt", 0777);      
    ...
    $NomArchCFDI = $SendaCFDI."PreCFDI-33_Factura_ComExt_".$NoFac.".xml";
...
XML;
    ...
    $NomArchSoap = $SendaCFDI."DatosEnvio_Factura_ComExt_".$NoFac.".xml";
...
## 13. PROCESOS POSTERIORES AL TIMBRADO ########################################
    ...
    $VarXML->save($SendaCFDI."RespServ_Factura_ComExt_".$NoFac.".xml");
    chmod($SendaCFDI."RespServ_Factura_ComExt_".$NoFac.".xml", 0777);
    ...
    foreach($RespServ as $Nodo){
        ...
        if($valor_del_nodo != ""){
            ...
            $NomArchXML = "CFDI-33_Factura_ComExt_".$NoFac.".xml";
            $NomArchPDF = "CFDI-33_Factura_ComExt_".$NoFac.".pdf";
...
//                file_put_contents($file_name, "\xEF\xBB\xBF".$f);
            ...
            $docXML->load($SendaCFDI."CFDI-33_Factura_ComExt_".$NoFac.".xml");
