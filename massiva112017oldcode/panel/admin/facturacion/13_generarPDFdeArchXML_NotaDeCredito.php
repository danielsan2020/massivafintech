...
$NomArchXML       = "CFDI_3.3_NotCred.xml"; // Nombre del archivo .XML correspondiente a un CFDI versión 3.3
$NomArchPDF       = "CFDI_3.3_NotCred.pdf"; // Nombre del archivo .PDF que se construirá.
...
$Doc = utf8_decode("NOTA DE CRÉDITO");
    ...
    #== 2. Obteniendo datos del archivo .XML =========================================
        ...
        ####### ENCABEZADO DE LA FACTURA #############################################################
            ...
            $pdf->image($SendaArchsGraf."LogoSAT.jpg",$X+16.6, $Y+3.4 , 0, 0);
            $pdf->image($SendaArchsGraf."FondoTenue.jpg",$X+1, $Y+8.7 , 19.5, 18);


            $pdf->SetTextColor(7,100,30);
            $pdf->SetFont('arial','B',13);
            $pdf->SetXY($X+10.5,$Y+1.25+0.1);
            $pdf->Cell(1.5, 0.25, $Doc, 0, 1,'L', 0);

                $pdf->SetTextColor(0,0,0);
                $pdf->SetFont('arial','B',10);
                $pdf->SetXY($X+15.5,$Y+1.46+0.36);
                $pdf->Cell(2.5, 0.25, "FOLIO Y SERIE:", 0, 1,'R', 0);

                $pdf->SetTextColor(171,17,17);
                $pdf->SetFont('arial','',14);
                $pdf->SetXY($X+18,$Y+1.45+0.36);
            ...
            $pdf->Cell(2, 0.25, $noCertificado, 0, 1,'L', 0);
                ...
                $pdf->MultiCell(9.4, 0.35, utf8_decode($Fact_Fecha), 0, 'L');
                ...
                $pdf->Cell(2, 0.25, $FechaCert, 0, 1,'L', 0);
        ...
        //#== Registros de artículos ================================================
            ...
            for ($i=0; $i<$TotRegs; $i++){
                    ...
                    if ($Puntero>23.5){
                        ...
                        if ($TotRegs>$Regs){
                            ...
                            $pdf->AddPage();
                            $pdf->image($SendaArchsGraf."FondoTenue.jpg",1, 5 , 19.5, 18);
                            Titulos($pdf, 1.5);
                            VerifStatusCFDI($pdf, $StatusCFDI);
                            $Y = 2.4;
                        }else{
            ...
            if ($Puntero>=18.5 && $Puntero<23.5){
                ...
                $pdf->AddPage();
                $pdf->image("archs_graf/$IdEmpr/FondoTenue.jpg",1, 5 , 19.5, 18);        
...
function VerifStatusCFDI($pdf, $StatusCFDI){
    ...
    if ($StatusCFDI=="CANCELADO"){
        ...
        $pdf->RoundedRect(4.4, 12.9, 12.6, 2.05, 0.4, '');
        $pdf->SetXY(1,13.9);
