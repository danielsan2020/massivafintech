...
$("#nuevoTicket").click(function(){
    ...
    if(id_usuario_reporta != '' && id_categoria_ticket != '' && titulo != ''){
        ...
        $.ajax({
            data: {accion,id_usuario_reporta,id_categoria_ticket,titulo,descripcion,conteo},
            url: "controlador/ticketContableControlador.php",
            type: 'POST',
            success: function (response){
                console.log(response);
                $("#alertAccion").append('<div class="alert alert-warning text-center"><b>Se ha creado su nuevo ticket, en breve atenderemos tu solicitud.</b></div>');
                $('#tnuevoElemento').modal('hide');
                window.setTimeout('location.reload()', 2000);
            },
            error: function(response){
                $("#alertAccion").append('<div class="alert alert-danger text-center">Ocurrió un error, por favor de verificar sus datos.</div>');
                $('#tnuevoElemento').modal('hide');
                window.setTimeout('location.reload()', 2000);
            }
        })
        ...
        $('#mensaje').html('<div class="alert alert-danger" role="alert">Verificar los campos requeridos.</div>');
    }
...
$("#terminarTicke").click(function(){
    ...
    $.ajax({
        ...
        success: function (response){
            console.log(response);
            $("#alertAccion").append('<div class="alert alert-warning text-center">Finalizó tu ticket.</div>');
            $('#termina').modal('hide');
            window.setTimeout('location.reload()', 2000);
        },
        error: function(response){
...
$("#agregaResClien").click(function(){
    ...
    if(comenCli != ''){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log(response);
                $("#alertAccion").append('<div class="alert alert-warning text-center"><b>Se agregó su comentario, en breve recibirá respuesta.</b></div>');
                window.setTimeout('location.reload()', 2000);
            },
            error: function(response){
                $("#alertAccion").append('<div class="alert alert-danger text-center">Ocurrió un error, por favor verificar tus datos.</div>');
        ...
        $('#alertAccion').html('<div class="alert alert-danger" role="alert">Escribir la respuesta</div>');
    }
    ...
    if(comentarioAdmin != ''){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log(response);
                $('#nuevoComentario').modal('hide');
                $("#alertAccion").append('<div class="alert alert-warning text-center"><b>Se agregó su comentario.</b></div>');
        ...
        $('#alertAccion').html('<div class="alert alert-danger" role="alert">Escribir sus comentarios</div>');
    }
...
$("#terminarTickeADmin").click(function(){
 ...
 /**********************************************************************************************/
/****************************************enviar correo de soporte contable*************************************/
$("#envioCorreo").click(function(){
    /*obtengo valores*/
    var accion = 'enviarcorreo';
    var idReportar = $('#idReportar').val();

    $.ajax({
        data: {accion,idReportar},
        url: "controlador/ticketContableControlador.php",
        type: 'POST',
        success: function (response){
            console.log(response);
            $("#alertAccion").append('<div class="alert alert-warning text-center">Se envió la asignación.</div>');
