...
function getChrCurrency($currencySignAsString)
{
	$getChrOfCur = array("€"=>128,
						 "¤"=>164,
						 "$"=>36,
						 "¢"=>162,
						 "£"=>163,
	...
	if(array_key_exists($currencySignAsString,$getChrOfCur)){
		return 	$getChrOfCur[$currencySignAsString];
	}else{
		return 	$currencySignAsString;
	}
...
{
	//$this->Image('../images/pdf/head_logo.jpg',8,6,196);
	//To be implemented in your own inherited class
}

function Footer($footer = NULL)
{
	//$this->Image('../images/pdf/footer.jpg',10,270,190);
	//To be implemented in your own inherited class
}
...
function SetTxtStyles($st)
{
    //Set the array of column alignments
    $this->tstyles=$st;
}

function SetTxtSizes($ss)
{
    //Set the array of column alignments
    $this->tsizes=$ss;
}
...
function Row($data,$rowStartX,$drawBorder = true,$textFont = 'Arial',$textSize = 9)
{
    ...
    for($i=0;$i<count($data);$i++)
    {
		...
		$st=isset($this->tstyles[$i]) ? $this->tstyles[$i] : '';
		$ss=isset($this->tsizes[$i]) ? $this->tsizes[$i] : $textSize;
        //Save the current position
        $x=$this->GetX();
        $y=$this->GetY();
        //Draw the border
        if($drawBorder) $this->Rect($x,$y,$w,$h);
        //Print the text
        $this->SetFont($textFont,$st,$ss);
