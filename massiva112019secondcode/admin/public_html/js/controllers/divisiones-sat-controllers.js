/*
 Controlador Lista Divisiones sat     
 */
/* global angular */

angular.module('app').controller("DivisionesSatListCtrl", [
    '$scope',
    'DivisionesSatServices',
    function ($scope, DivisionesSatServices) {
        $scope.divisiones_sat = [];
        var tipos = [
            {id: "1", tipo: 'Productos'},
            {id: "2", tipo: 'Servicios'}
        ];

        DivisionesSatServices.getList().then(function (data) {
        ...
        $scope.inactivate = function (id) {
            DivisionesSatServices.inactivate(id).then(function () {
                DivisionesSatServices.getList().then(function (data) {
                    $scope.divisiones_sat = data.divisiones_sat;
                });
            });
