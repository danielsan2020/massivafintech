...
class Personas_contadores extends REST_Controller {
    ...
    public function get_all_declaraciones_by_contador_id_get() {
        ...
        $data['declaraciones'] = $this->personas_contadores_model->get_all_declaraciones_by_contador_id($contador_id);
        $this->response($data, REST_Controller::HTTP_OK);
    }
    ...
    public function get_all_declaraciones_atrasadas_by_contador_id_get() {
        ...
        $contador_id = $this->get('id');
        $this->load->helper('session');
        $data['declaraciones_atrasadas'] = $this->personas_contadores_model->get_all_declaraciones_declaraciones_contador_id($contador_id);
        $this->response($data, REST_Controller::HTTP_OK);
    }

    /**
     * @title Metodo para obtener una declaracion por medio de la persona id y la declaracion id
     * @url /personas_contadores/get_declaracion_by_persona_id_and_declaracion_id_get
     * @access public
     * @method GET
     * @successResponse Code: 200 HTTP_OK
     */
    public function get_declaracion_by_persona_id_and_declaracion_id_get() {
        $this->_validation_persona_id_and_declaracion_id();
        $persona_id = $this->get('persona_id');
        $declaracion_id = $this->get('declaracion_id');
        $this->load->helper('session');
        $contador_id = check_id();
        $this->_validation_datos_de_la_peticion_get_declaracion($persona_id, $declaracion_id, $contador_id);
     ...
     * @successResponse Code: 200 HTTP_OK
     */
    public function get_declaracion_atrasada_by_persona_id_and_declaracion_id_get() {
        $this->_validation_persona_id_and_declaracion_id();
        $persona_id = $this->get('persona_id');
        $declaracion_id = $this->get('declaracion_id');
        $this->load->helper('session');
        $contador_id = check_id();
        $this->_validation_datos_de_la_peticion_get_declaracion_atrasada($persona_id, $declaracion_id, $contador_id);
        $data['declaracion'] = $this->declaraciones_atrasadas_model->get_declaracion_by_persona_id_and_declaracion_id($persona_id, $declaracion_id);
        $this->response($data, REST_Controller::HTTP_OK);
    }
    ...
    private function _validation_persona_id_and_declaracion_id() {
        $this->load->library('params_validation');
        $this->params_validation->set_rules('persona_id', 'Id', 'trim|required|is_natural_no_zero');
        $this->params_validation->set_rules('declaracion_id', 'Id', 'trim|required|is_natural_no_zero');
        if (!$this->params_validation->run()) {
            $this->response(['message' => 'datos invalidos para realizar peticion'], REST_Controller::HTTP_BAD_REQUEST);
        }
    }

    private function _validation_datos_de_la_peticion_get_declaracion($persona_id, $declaracion_id, $contador_id) {
    ...
    private function _validation_datos_de_la_peticion_get_declaracion_atrasada($persona_id, $declaracion_id, $contador_id) {
        if (!($this->_existe_relacion_entre_persona_and_contador($persona_id, $contador_id))) {
            $this->response(['message' => 'la persona no esta asignada al contador'], REST_Controller::HTTP_BAD_REQUEST);
        }

        if (!($this->_es_la_declaracion_atrasada_de_la_persona($persona_id, $declaracion_id))) {
            $this->response(['message' => 'la declaracion no es de esa persona'], REST_Controller::HTTP_BAD_REQUEST);
        }
    }
    ...
    private function _es_la_declaracion_atrasada_de_la_persona($persona_id, $declaracion_id){
        $this->load->model('declaraciones_atrasadas_model');
        $relacion_persona_declaracion=$this->declaraciones_atrasadas_model->get_by_id_and_persona_id($declaracion_id, $persona_id);
        if ((count($relacion_persona_declaracion) === 0)){
            return false;
        }
        return true;
