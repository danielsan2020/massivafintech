...
class Soporte_comentarios extends REST_Controller {
    ...
    /**
     * @title Metodo para obtener todos los registros activos de la tabla soporte_comentarios
     * @url /paquetes/get_all_get
     * @access public
     * @method GET
     * @successResponse Code: 200 HTTP_OK
     */
    public function get_all_comentarios_by_soporte_ticket_id_get() {
        $this->_params_validation_soporte_ticket_id();
        $soporte_ticket_id = $this->get('soporte_ticket_id');
        $data['lista_comentarios'] = $this->soporte_comentarios_model->get_all_by_soporte_ticket_id($soporte_ticket_id);
     ...
     */
    public function create_texto_post() {
        $this->_params_validation_soporte_ticket_id();
    ...
    public function create_file_post() {
        ...
        $this->load->helper('logs');
        insert_log($id, 'soporte_comentarios', $data);
        $this->_upload_file($data['soporte_ticket_id'], $id);
        $this->db->trans_complete();
        $this->response(['id' => $id, 'message' => 'Comentario creado'], REST_Controller::HTTP_CREATED);
    }
     ...
     * @errorResponse Code: 500 HTTP_INTERNAL_SERVER_ERROR Content: {message:[string]}
     */
    private function _upload_file($soporte_ticket_id, $comentario_id) {
